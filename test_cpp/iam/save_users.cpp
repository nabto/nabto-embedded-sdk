...
BOOST_AUTO_TEST_CASE(user_to_json)
{
    ...
    user.id = strdup("1");
    nn_string_set_insert(&user.roles, "role1");
    user.fingerprint = strdup("fp");
    user.serverConnectToken = strdup("sct");
    nn_string_map_insert(&user.attributes, "key", "value");
    ...
    BOOST_TEST(nn_string_set_size(&user.roles) == nn_string_set_size(&decoded->roles));
    BOOST_TEST(nn_string_map_size(&user.attributes) == nn_string_map_size(&decoded->attributes));
