...
BOOST_AUTO_TEST_CASE(event_test)
{
    ...
    struct nabto_device_event_handler* handler = nabto_device_event_handler_new(dev, nabto::test::eventHandlerCallback, &listener);
    BOOST_TEST(handler);
    np_error_code ec = nabto_device_event_handler_add_event(handler, &event);
...
BOOST_AUTO_TEST_CASE(event_test_multi_events)
{
    ...
    struct nabto_device_event_handler* handler =
        nabto_device_event_handler_new(dev, nabto::test::eventHandlerCallback, &listener);
    BOOST_TEST(handler);
    np_error_code ec = nabto_device_event_handler_add_event(handler, &event1);
    BOOST_TEST(ec == NABTO_EC_OK);
    ec = nabto_device_event_handler_add_event(handler, &event2);
    BOOST_TEST(ec == NABTO_EC_OK);
    ...
    ec2 = nabto_device_event_handler_create_future((NabtoDeviceEventHandler*)handler, &fut);
    BOOST_TEST(ec2 == NABTO_DEVICE_EC_OK);
    nabto_api_future_queue_execute_all(&dev->queueHead);
    BOOST_TEST(nabto_device_future_ready(fut) == NABTO_DEVICE_EC_OK);
    BOOST_TEST(nabto_device_future_error_code(fut) == NABTO_DEVICE_EC_OK);
