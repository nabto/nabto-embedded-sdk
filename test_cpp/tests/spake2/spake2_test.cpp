...
BOOST_AUTO_TEST_CASE(calculate_mbedtls_key_deterministically, * boost::unit_test::timeout(120))
{
    ...
    std::vector<uint8_t> T;
    BOOST_TEST(cli.calculateTWithCustomEntropy(T, nabto::test::xorshift_entropy_func) == 0);
    ...
    BOOST_TEST(nm_mbedtls_spake2_calculate_key(NULL, req, nabto::test::xorshift_entropy_func, password.c_str(), S, &SLen, key) == NABTO_EC_OK);

    // this reference output was calculated with mbedtls + xorshift32 entropy on Ubuntu 22.04.4 LTS
    uint8_t expectedKey[] = {
        0x8E, 0xF8, 0xE8, 0x91, 0x6F, 0xF9, 0xD3, 0x14,
        0x5D, 0xAA, 0x04, 0x84, 0x59, 0x2F, 0x99, 0xD6,
        0xEF, 0x9A, 0x84, 0x20, 0x61, 0x62, 0xD1, 0x65,
        0x22, 0xA8, 0xE4, 0x1A, 0x40, 0x0F, 0x25, 0x79
    };
