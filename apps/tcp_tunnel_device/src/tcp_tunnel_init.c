...
bool create_state_interactive(struct nm_file* fileImpl, const char* file);
bool create_state_interactive_custom(struct nm_file* fileImpl, const char* file);
bool create_state_default(struct nm_file* fileImpl, const char* file);
bool create_services_interactive(struct nm_file* fileImpl, const char* file);

static bool prompt_create_device_config(struct tcp_tunnel* tcpTunnel)
{
    if (string_file_exists(&tcpTunnel->fileImpl, tcpTunnel->deviceConfigFile)) {
        ...
        if (!create_device_config_interactive(&tcpTunnel->fileImpl, tcpTunnel->deviceConfigFile)) {
            return false;
        }
    }
    printf(NEWLINE);
...
bool tcp_tunnel_config_interactive(struct tcp_tunnel* tcpTunnel) {
    ...
    if (string_file_exists(&tcpTunnel->fileImpl, tcpTunnel->iamConfigFile)) {
    ...
    if (createIamConfig) {
        if (!iam_config_create_default(&tcpTunnel->fileImpl, tcpTunnel->iamConfigFile)) {
    ...
    if (string_file_exists(&tcpTunnel->fileImpl, tcpTunnel->stateFile)) {
    ...
    if (createIamState) {
        if (!create_state_interactive(&tcpTunnel->fileImpl, tcpTunnel->stateFile)) {
    ...
    if (string_file_exists(&tcpTunnel->fileImpl, tcpTunnel->servicesFile)) {
    ...
    if (createServices) {
        if (!create_services_interactive(&tcpTunnel->fileImpl, tcpTunnel->servicesFile)) {
...
bool create_state_interactive(struct nm_file* fileImpl, const char* file)
{
    ...
    if (createCustomIam) {
        ...
        return create_state_interactive_custom(fileImpl, file);
    } else {
        printf("Use default iam" NEWLINE);
        return create_state_default(fileImpl, file);
    }
}
...
bool create_state_interactive_custom(struct nm_file* fileImpl, const char* file) {
    ...
    return save_tcp_tunnel_state(fileImpl, file, state);
...
bool create_services_interactive(struct nm_file* fileImpl, const char* file)
{
    ...
    if (createCustom) {
        ...
        return json_config_save(fileImpl, file, root);
    } else {
        printf("Use default services" NEWLINE);
        return tcp_tunnel_create_default_services_file(fileImpl, file);
    }
}
...
bool tcp_tunnel_demo_config(struct tcp_tunnel* tcpTunnel)
{
    ...
    if (string_file_exists(&tcpTunnel->fileImpl, tcpTunnel->iamConfigFile) ||
        string_file_exists(&tcpTunnel->fileImpl, tcpTunnel->stateFile) ||
        string_file_exists(&tcpTunnel->fileImpl, tcpTunnel->servicesFile))
    {
...

    {
        ...
        save_tcp_tunnel_state(&tcpTunnel->fileImpl, tcpTunnel->stateFile, state);
    }
    ...
    printf(NEWLINE);
    json_config_save(&tcpTunnel->fileImpl, tcpTunnel->servicesFile, root);
