...
static bool write_state_to_file(struct nm_file* fileImpl, const char* stateFile, struct nm_iam_state* state);

bool load_tcp_tunnel_state(struct nm_iam_state* state, struct nm_file* fileImpl, const char* stateFile, struct nn_log* logger)
{
    char* str;
    if (!string_file_load(fileImpl, stateFile, &str)) {
        return false;
    }
    ...
    if (state->friendlyName == NULL) {
        ...
        write_state_to_file(fileImpl, stateFile, state);
    }
    return true;
}

bool write_state_to_file(struct nm_file* fileImpl, const char* stateFile, struct nm_iam_state* state)
{
    char* str;
    ...
    if(!string_file_save(fileImpl, stateFile, str)) {
...
{
    return write_state_to_file(fileImpl, stateFile, state);
}
