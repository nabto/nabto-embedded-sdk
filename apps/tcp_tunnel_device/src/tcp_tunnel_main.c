...
struct tcp_tunnel* tcp_tunnel_new()
{
    ...
    tunnel->fileImpl = nm_file_unix_get_impl();
...
bool handle_main(struct args* args, struct tcp_tunnel* tunnel)
{
    ...
    if (args->init) {
        if (!load_or_create_private_key(tunnel->device, &tunnel->fileImpl, tunnel->privateKeyFile, &logger)) {
    ...
    if (!load_device_config(&tunnel->fileImpl, tunnel->deviceConfigFile, &dc, &logger)) {
    ...
    if (!iam_config_load(tunnel->iamConfig, &tunnel->fileImpl, tunnel->iamConfigFile, &logger)) {
        print_iam_config_load_failed(tunnel->iamConfigFile);
        return false;
    }


    if (!load_tcp_tunnel_state(tunnel->tcpTunnelState, &tunnel->fileImpl, tunnel->stateFile, &logger)) {
        print_tcp_tunnel_state_load_failed(tunnel->stateFile);
        return false;
    }

    if (!load_tcp_tunnel_services(&tunnel->services, &tunnel->fileImpl, tunnel->servicesFile, &logger))
    {
...
void iam_user_changed(struct nm_iam* iam, void* userData)
{
    ...
    } else if (!save_tcp_tunnel_state(&tcpTunnel->fileImpl, tcpTunnel->stateFile, state)) {
