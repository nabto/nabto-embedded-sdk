...
bool create_private_key(NabtoDevice* device, struct nm_file* fileImpl, const char* filename, struct nn_log* logger)
{
    ...
    bool status = string_file_save(fileImpl, filename, privateKey);
...
{
    if (!string_file_exists(fileImpl, filename)) {
        if (!create_private_key(device, fileImpl, filename, logger)) {
    ...
    char* privateKey;
    bool status = string_file_load(fileImpl, filename, &privateKey);
    if (status == false) {
