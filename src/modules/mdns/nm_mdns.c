...
static void nm_mdns_send_packet_v4(struct nm_mdns* mdns, struct np_udp_endpoint ep);
                             ...
                             np_communication_buffer* buffer, uint16_t bufferSize, void* userData)
{
    ...
    if (ec == NABTO_EC_OK) {
        ...
        {
            nm_mdns_send_packet_v4(mdns, ep);
            return;
        }
    }
    nm_mdns_recv_packet_v4(mdns);
...
void nm_mdns_send_packet_v4(struct nm_mdns* mdns, struct np_udp_endpoint ep)
{
    struct np_platform* pl = mdns->pl;
    size_t written;
