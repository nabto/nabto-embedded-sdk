...
static void v6_closed(np_error_code ec, void* userData);
static void v4_closed(np_error_code ec, void* userData);
...
np_error_code nm_mdns_server_init(struct nm_mdns_server* server, struct np_event_queue* eq, struct np_udp* udp, struct nm_mdns_udp_bind* mdnsUdpBind, struct np_local_ip* localIp)
{
    ...
    np_error_code ec = instance_init(&server->v4, server, true);
    if (ec != NABTO_EC_OK) {
        return ec;
    }
