...
{
    const char* paths[] = { "iam", "users", "{user}", "fcm-token",  NULL };
...
void handle_request(struct nm_iam_coap_handler* handler, NabtoDeviceCoapRequest* request)
{
    ...
    char* fcmToken = NULL;
    if (!nm_iam_cbor_decode_string(&value, &fcmToken) || fcmToken == NULL) {
        nabto_device_coap_error_response(request, 400, "Bad request");
        return;
    }
    ...
    if (!nm_iam_internal_check_access(handler->iam, nabto_device_coap_request_get_connection_ref(request), "IAM:SetUserFcmToken", &attributes)) {
    ...
    } else if (!nm_iam_user_set_fcm_token(user, fcmToken)) {
