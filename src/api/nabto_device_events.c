...
const int NABTO_DEVICE_EVENT_UNKNOWN_FINGERPRINT   = (int)NC_DEVICE_EVENT_UNKNOWN_FINGERPRINT;
...
const int NABTO_DEVICE_EVENT_PLATFORM_FATAL_FAILURE   = (int)NC_DEVICE_EVENT_PLATFORM_FATAL_FAILURE;
...
void nabto_device_events_core_cb(enum nc_device_event event, void* userData)
{
    ...
    struct nabto_device_listen_device_event* ev = (struct nabto_device_listen_device_event*)np_calloc(1, sizeof(struct nabto_device_listen_device_event));
    if (ev == NULL) {
...
NabtoDeviceError NABTO_DEVICE_API nabto_device_device_events_init_listener(NabtoDevice* device, NabtoDeviceListener* deviceListener)
{
    ...
    struct nabto_device_listen_device_context* ctx = (struct nabto_device_listen_device_context*)np_calloc(1, sizeof(struct nabto_device_listen_device_context));
    if (ctx == NULL) {
