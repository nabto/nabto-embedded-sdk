...
{
    obj->vptr->async_connect(sock, address, port, completionEvent);
}
...
{
    obj->vptr->async_write(sock, data, dataLength, completionEvent);
}
...
{
    obj->vptr->async_read(sock, buffer, bufferLength, readLength, completionEvent);
}
void np_tcp_shutdown(struct np_tcp* obj, struct np_tcp_socket* sock)
{
    obj->vptr->shutdown(sock);
}
