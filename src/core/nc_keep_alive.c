...
void nc_keep_alive_deinit(struct nc_keep_alive_context* ctx)
{
    if (ctx->pl != NULL) { // if init called
        np_event_queue_cancel_timed_event(&ctx->keepAliveEvent);
    }
}
...
{
    np_event_queue_init_timed_event(ctx->pl, &ctx->keepAliveEvent, cb, data);
    np_event_queue_post_timed_event(&ctx->keepAliveEvent, ctx->kaRetryInterval);
}
