...
{
    if (module->passwordRequest != NULL) {
        return NABTO_EC_INVALID_STATE;
    }
    module->passwordRequest = passwordRequestFunction;
    module->passwordRequestData = data;
    return NABTO_EC_OK;
}
...
int hashMpi(mbedtls_md_context_t* mdCtx, mbedtls_mpi* n)
{
    ...
    uint8_t buffer[256];
    int status = 0;
    status |= mbedtls_mpi_write_binary (n, buffer, sizeof(buffer));
