...
np_error_code nc_coap_server_init(struct np_platform* pl, struct nc_coap_server_context* ctx)
{
    ...
    np_event_queue_init_event(pl, &ctx->ev, &nc_coap_server_notify_event_callback, ctx);

    np_event_queue_init_timed_event(ctx->pl, &ctx->timer, &nc_coap_server_handle_timeout, ctx);
