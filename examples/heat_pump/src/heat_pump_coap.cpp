...
void heat_pump_set_power(NabtoDeviceCoapRequest* request, void* userData)
{
    ...
    if (!application->checkAccess(request, "HeatPump:Set")) {
        return;
    }
...
{
    HeatPump* application = (HeatPump*)userData;
    if (!application->checkAccess(request, "HeatPump:Get")) {
        return;
    }
...
{
    HeatPump* application = (HeatPump*)userData;
    if (!application->checkAccess(request, "Beta:GetClientSettings")) {
        return;
    }
