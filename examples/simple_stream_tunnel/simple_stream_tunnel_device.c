...
static void listen_for_stream(NabtoDeviceListener* listener,
                       NabtoDeviceFuture* future);
static void listen_for_authorization_requests(NabtoDeviceListener* listener,
                                       NabtoDeviceFuture* future);
...
void new_stream_callback(NabtoDeviceFuture* future, NabtoDeviceError ec, void* userData)
{
    ...
    listen_for_stream((NabtoDeviceListener*)userData, future);
}

void listen_for_stream(NabtoDeviceListener * listener, NabtoDeviceFuture* future ) {
    nabto_device_listener_new_stream(listener, future, &head.stream);
    nabto_device_future_set_callback(future, &new_stream_callback, listener);
}

void new_authorization_request_callback(NabtoDeviceFuture* future, NabtoDeviceError ec, void* userData)
{
    if (ec != NABTO_DEVICE_EC_OK) {
        return;
    }
    nabto_device_authorization_request_verdict(head.authorizationRequest, true);
    nabto_device_authorization_request_free(head.authorizationRequest);
    head.authorizationRequest = NULL;
    listen_for_authorization_requests((NabtoDeviceListener*)userData, future);
}

void listen_for_authorization_requests(NabtoDeviceListener * listener, NabtoDeviceFuture* future ) {
    nabto_device_listener_new_authorization_request(listener, future, &head.authorizationRequest);
    nabto_device_future_set_callback(future, &new_authorization_request_callback, listener);
}
...
int main(int argc, char** argv)
{
    ...
    if ((listener = nabto_device_listener_new(device)) == NULL ||
        ...
        (authorizationListener = nabto_device_listener_new(device)) == NULL ||
        (authorizationFuture = nabto_device_future_new(device)) == NULL ||
        nabto_device_authorization_request_init_listener(device, authorizationListener) != NABTO_DEVICE_EC_OK)
    {
        handle_device_error(device, listener, "Failed to initialize");
        return -1;
    }

    ec = nabto_device_add_tcp_tunnel_service(device, ssh.id, ssh.type, ssh.host, ssh.port);
    if (ec != NABTO_DEVICE_EC_OK) {
        handle_device_error(device, listener, "Failed to add tunnel service");
        return -1;
    } else {
            printf("Added a TCP Tunnel service. Id: %s, type: %s, host: %s, port: %d" NEWLINE,
                   ssh.id, ssh.type, ssh.host, ssh.port);
        }

    signal(SIGINT, &signal_handler);

    listen_for_stream(listener, listenerFuture);
    listen_for_authorization_requests(authorizationListener,
                                      authorizationFuture);

    printf("Press any key to close the device" NEWLINE);
    getchar();
